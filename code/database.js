...
function openDatabase(type, comparison, shh) {
    ...
    if (!comparison) {
        HTMLContent += `
        <div>
            <div class='container' style='gap:10px;margin:10px 0'>`;
        ...
        HTMLContent += `
            ...
            <div id='allButtonDatabase' class='button cuphead' onclick="databaseCategory=null;fetchDatabase('${type}',null,true)">ALL</div>
            <div id='databaseCategorySelect'>${categorySelect(true)}</div>
        </div>`
    }
    HTMLContent += `
    ...
    <table class='border' style='margin:20px;padding:10px'>`
    let empty = true
